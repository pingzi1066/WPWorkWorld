...
namespace KMTool
{
    ...
    public class XLS_EditorDrawHelper {

        public static void DrawExcelTab(Dictionary<string,Excel> dict , ref int selectIndex)
        {
            GUILayout.BeginHorizontal();
            {
                for (int i = 0; i < dict.Count; i++)
                {
                    ...
                    if (GUILayout.Toggle(selectIndex == i, key, EditorStyles.toolbarButton))
                    {
                        selectIndex = i;
                    }
                }
        ...
        public static void DrawTableTab(Excel xls, ref int selectIndex)
        {
            GUILayout.BeginHorizontal();
            {
                if (selectIndex >= xls.Tables.Count)
                    selectIndex = 0;
    ...
    //        if (!(cell is String))
    //        {
    //            Debug.Log("not string row " + row + " column " + column);
    //        }
        }
