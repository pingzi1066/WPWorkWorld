...
public class WayPointBezier : MonoBehaviour
{
    ...
    [SerializeField]
    private WayPoint[] _controlPoints = new WayPoint[0];
    ...
    //added by wp: upgrade to unity 4 and method
    ...
    public void DeletePoint(WayPoint deletePoint, bool destroy)
    {
    ...
    {
        List<WayPoint> rebuildList = new List<WayPoint>(_controlPoints);
        WayPoint deletePoint = rebuildList[index];
        rebuildList.RemoveAt(index);
        _controlPoints = new WayPoint[rebuildList.Count];
        _controlPoints = rebuildList.ToArray();
        if (destroy)
            DestroyImmediate(deletePoint.gameObject);
        RecalculateStoredValues();
    ...
    {
        return AddNewPoint(_controlPoints.Length);
    }
    ...
    public GameObject AddNewPoint(int index)
    {
        ...
        else
        {
            ...
            {
                WayPoint pointA = _controlPoints[Mathf.Clamp(index - 1, 0, numberOfUseablePoints)];
                WayPoint pointB = _controlPoints[Mathf.Clamp(index, 0, numberOfUseablePoints)];
    ...
    public float GetPathPercentageAtPoint(WayPoint point)
    {
        List<WayPoint> cps = new List<WayPoint>(_controlPoints);
